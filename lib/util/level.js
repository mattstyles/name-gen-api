...
class DB extends EventEmitter {
    constructor() {
        ...
        // Add meta collection
        this.subs.add( 'meta', promisify( sub.sublevel( 'meta', {
            encoding: 'json'
        })));

        // Add other categories
        this.subs.meta.get( 'categories' )
            .then( res => {
                if ( !res.length ) {
    ...
    createCategory( name ) {
        return new Promise( ( resolve, reject ) => {
            ...
            [ 'common', 'uncommon', 'rare' ].forEach( commonality => {
                var c = cat.root.sublevel( commonality, {
                    encoding: 'json'
                });
            ...
            this.subs.meta.get( 'categories' )
                .then( res => {
                    ...
                    this.subs.meta.put( 'categories', res )
                        .then( () => {
                            resolve( cat );
                        })
                        .catch( reject );
